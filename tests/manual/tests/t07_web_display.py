...
def run(ctx: TestContext) -> None:
    ...
    with tempfile.TemporaryDirectory() as _tmp:
        ...
        display_ctx = DisplayContext(width=800, height=480, fade_duration_s=0.0, dwell_s=2.0)
    ...
    if not ctx.no_interactive:
        ans = ctx.prompt("Did an orange web page with 'malmberg' heading appear for ~2s?")
